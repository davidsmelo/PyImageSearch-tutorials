...
ap.add_argument("-m", "--model", required=True, help="Path to DL model")
ap.add_argument("-dt", "--dataset-test",required=True, help= "Path to test images dataset")
ap.add_argument("-b", "--batch-size", type=int, default=32, help="size of mini-batches passed to network")

args=vars(ap.parse_args())
CLASSES = ["cat", "dog"]

print("[INFO] loading architectures and weights...")
model = load_model(args["model"])
print ("[INFO] testing on images in {}".format(args["dataset_test"]))

for imagePath in paths.list_images(args["dataset_test"]):
